...
void MainWindow::initGraph3D() {
    ...
    axisY->setLabelFormat("%.1f米");
    // axisY->setRange(-5000, 5000);
    ...
    //添加数据
    QImage heightMapImage(":/images/height.jpg");

    //用代理添加数据
    proxy->setHeightMap(heightMapImage);
    ...
    ui->surfaceComboBox->setCurrentIndex(2);
}
...
void MainWindow::on_selectComboBox_currentIndexChanged(int index) {
    if (index == 0) {
        surfaceChart->setSelectionMode(QAbstract3DGraph::SelectionFlag(0));
    } else if (index == 1) {
        surfaceChart->setSelectionMode(QAbstract3DGraph::SelectionFlag(index));
    } else if (index == 2) {
        surfaceChart->setSelectionMode(QAbstract3DGraph::SelectionFlag(3)|QAbstract3DGraph::SelectionSlice);
    } else if (index == 3) {
        surfaceChart->setSelectionMode(QAbstract3DGraph::SelectionFlag(5)|QAbstract3DGraph::SelectionSlice);
    }
}
